...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        Node<K,V> *z= new Node<K,V>(key,key_value.second);
            while(x != nullptr){
                y=x;
            //checks if key value is less than x and not a duplicate 
                if(z->key() == x->key()){
                    std::ostringstream oss;
                    oss<<"Attempt to insert duplicate key '"<< z->key()<<"'.";
                    delete z;
        ...
        z->parent = y;
        // key value < x then check if y is null.
        if(y == nullptr){
            root_ = z;
        }
        else if(z->key() < y->key()){
...
private:
    ...
    int height(Node<K, V> *node) const {
        ...
        //keep moving down, take max of the sub trees plus one
        else{
    ...
    size_t leaf_count(Node<K, V> *node) const {
        ...
        //internal node so check if children are leafs
        else{
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        ...
        // if no children then leaf, so not an internal node
        else if(node->right==nullptr && node->left==nullptr){
            return 0;
        }
        //is an internal node, must also check children
        else{
    ...
    int diameter(Node<K, V> *node) const {
        ...
        //find the length of the left and right sub trees, they are the longest path possible through the node
        int left = 1 + height(node->left);
        int right = 1 + height(node->right);
        int sum =left + right;
        //find the largest diameter going through each child
        int childd= std::max(diameter(node->right),diameter(node->left));
        return std::max(sum,childd);
    }
    ...
    size_t null_count(Node<K, V> *node) const {
        // TODO
        //reach a null node, so add
        if(node==nullptr){
            return 1;
        }
        //not a null so check children
        else{
