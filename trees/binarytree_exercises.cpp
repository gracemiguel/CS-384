...
Node* lowest_common_ancestor(Node *p, Node *q) {
    if(p == nullptr || q == nullptr){       //checks to see if either node is null
        return nullptr;
    }
    if( p ==q){
        return p;
    }
    if(p->level > q->level){                        //checking to see if one Node is deeper than the other
...
bool is_bst(Node *n, int *min, int *max) {
    ...
    return is_bst(n->left, min, &n->data) && is_bst(n->right, &n->data, max);  // the  & gives the address, you want the address because the *max is the pointer, not the value
